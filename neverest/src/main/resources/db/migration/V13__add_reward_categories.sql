-- V13: Reward categories live in the DB (no more hardcoded mapping in the app).
ALTER TABLE nev_rewards
    ADD COLUMN category VARCHAR(40) NULL AFTER image_b64;

-- Category definitions (code + labels). Used for filter chips / admin pickers.
...
-- Backfill existing seed rewards with their category (matches the old hardcoded rules).
...
-- Any remaining rewards without a category default to PARTNER.
